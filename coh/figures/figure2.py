...
def makeFigure():
    ...
    ax[6].set(ylim=(-200, 1000))

    # IFNg by status
    CoH_DF = pd.read_csv("./coh/data/CoH_Flow_DF.csv")
    CoH_DF = CoH_DF.loc[CoH_DF.Time == "15min"]
    ligs = ["IFNg-50ng", "IL10-50ng"]
    axes = [7, 9]

    for i, sigg in enumerate(["pSTAT1", "pSTAT3"]):
...
def cytok_stim_plot(CoH_DF, cytok, cells, ax):
    ...
    sns.barplot(data=CoH_DF, x="Cell", y="Mean", hue="Marker", palette='husl', ax=ax)
