...
def CoH_comp_scan_plot(ax, tFac, CoH_Array, status_DF):
    ...
    for comps in itertools.product(all_comps, all_comps):
        ...
        Acc_DF = pd.concat([Acc_DF, pd.DataFrame({"Component 1": "Comp. " + str(comps[0] + 1), "Component 2": "Comp. " + str(comps[1] + 1), "Accuracy": [LR_CoH.score(compFacs, Donor_CoH_y)]})])
    
    Acc_DF = Acc_DF.pivot_table(index='Component 1', columns='Component 2', values='Accuracy', sort=False)
    sns.heatmap(data=Acc_DF, vmin=0, vmax=1, xticklabels=True, yticklabels=True, cbar_kws={'label': 'Accuracy 10-fold CV'}, ax=ax)
